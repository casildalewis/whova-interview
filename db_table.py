...
# CHANGES : added constraints parameter in CREATE, added functionality for like in SELECT
# NOTE: changed iteritems to items
...
class db_table:
    ...
    # records table name, schema, and constraints
    ...
    # \param constraints    string                constraints of the DB table
    #
    ...
    # Create the database table based on self.name, self.schema and self.constraints
    ...
    def create_table(self):
        ...
        self.db_conn.execute("CREATE TABLE IF NOT EXISTS %s (%s%s)" % (self.name, columns_query_string, self.constraints))
        self.db_conn.commit()
    ...
    def select(self, columns = [], where = {}):
        ...
        if where:
            where_query_string = [ ("%s LIKE '%s'" % (k,v)) if ("%" in v) else ("%s = '%s'" % (k,v)) for k,v in where.items() ]
